...
class VTable extends Component {
  ...
  componentDidUpdate() {
    let myThis = this;
    if (myThis.state.style.height)
      $(myThis.myRef.current.rootTableElement).height(myThis.state.style.height);
    if (myThis.state.style.width)
      $(myThis.myRef.current.rootTableElement).width(myThis.state.style.width);
    ...
    if (myThis.state.tree.selectedRow && myThis.state.tree.selectedRow.length>0) {
        let selectedReference = myThis.state.tree.selectedRow[0].reference;
        newSelectedRow = rows.reduce((acc,row,index)=>(row.reference===selectedReference?index:acc));
    }

    myThis.setState({
      tree: myThis.props.tree,
      selectedRow: newSelectedRow,
      rowsCount: rows.length,
      empty: true // draw empty, then draw non-empty to refresh the cells
    }, () => myThis.setState({empty:false}, ()=>myThis.setState({empty:false})) );
  ...
  render() {
    ...
    let renderedCols = null;
    
    if (!this.state.empty)
      renderedCols = cols.map((col,j)=>{      
        let renderer = (i) => {
